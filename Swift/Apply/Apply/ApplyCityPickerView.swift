//
//  ApplyCityPickerView.swift
//  Apply
//
//  Created by 刘欢 on 2017/4/22.
...
class ApplyPickerViewConfig {
    ...
    init(pickerHeight: CGFloat = 260.f, isTapDismiss: Bool = true) {
        self.isTapDismiss = isTapDismiss
        self.pickerHeight = pickerHeight
    }
}

class ApplyPickerView {

    static let tapGes = UITapGestureRecognizer(target: ApplyPickerView.self, action: #selector(ApplyPickerView.dismiss))
    ...
    static func configer(config: (ApplyPickerViewConfig) -> ()) -> (ApplyPickerView.Type) {
        config(self.config)
        return self
    }
    ...
    static func showCityPickerView(confirmHandle: @escaping ConfirmHandle) {
        ...
        let picker = ApplyCityPickerView(height: height) {
            dismiss()
            if !$0 {
                confirmHandle($1, $2)
            }
...
class ApplyCityPickerView: UIView {
...
extension ApplyCityPickerView: UIPickerViewDataSource, UIPickerViewDelegate {
